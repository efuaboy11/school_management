...
import { SchoolFeesLineChart } from '@/components/chatFrames'
...
const AllSchoolFeesPayment = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = allSchoolFeesPaymentData.map((data) => data.id);
  ...
  // const allSchoolFeesPaymentData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = allSchoolFeesPaymentData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-payment-school-fees/', {
        method: 'POST',
        headers: {
      ...
      if(response.ok){
        ...
        setAllSchoolFeesPaymentData(allSchoolFeesPaymentData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
      ...
      <div className="container-xl pt-4 pb-5 mb-5">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/bursary/school-fees-payment/add/step-1' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i>Pay Fees</Link>
          </div>
        </div>
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === allSchoolFeesPaymentData.length && allSchoolFeesPaymentData.length > 0}
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>
                                <Link href={`/bursary/school-fees-payment/individual/${data.id}/${data.transaction_id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
