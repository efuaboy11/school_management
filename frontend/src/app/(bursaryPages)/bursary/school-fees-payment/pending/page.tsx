...
const PendingPayment = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = pendingSchoolFeesPaymentData.map((data) => data.id);
  ...
  const UpdateStatus = async(e:any) =>{
    ...
    try{
      ...
      if(response.ok){
        ...
        hideStatusModal()
        PendingSchoolFeesPaymentFunction()
        setPendingSchoolFeesPaymentData(pendingSchoolFeesPaymentData.filter(dat => dat.id !== selectedDataId))
      }else{
  ...
  const deleteFunction = async () =>{
    ...
    try{
      ...
      if(response.ok){
        setLoader(false)
        setDisableButton(false)
        setPendingSchoolFeesPaymentData(pendingSchoolFeesPaymentData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === pendingSchoolFeesPaymentData.length && pendingSchoolFeesPaymentData.length > 0}
