...
const SchoolFees = () => {
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-school-fees/', {
        method: 'POST',
        headers: {
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/bursary/school-fees-bill/create' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Create fee</Link>
          </div>
        </div>
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>
                                <Link href={`/bursary/school-fees-bill/school-fees/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
