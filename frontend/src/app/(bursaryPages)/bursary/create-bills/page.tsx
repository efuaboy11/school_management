...
const Bills = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = billsData.map((data) => data.id);
  ...
  // const billsData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = billsData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-bills/', {
        method: 'POST',
        headers: {
      ...
      if(response.ok){
        ...
        setBillsData(billsData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
    ...
    <div className='mb-5 pb-5'>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">Bills</p>
            <p className="light-text pb-3">Total of {billsCount} bills avaliable</p>
         </div>

          <div className='d-flex mb-4'>
            <Link href='/bursary/create-bills/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Add bill</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="site-search-input" placeholder="Search" value={billsSearch} onChange={(e) => setBillsSearch(e.target.value)}/>
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === billsData.length && billsData.length > 0}
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>
                                <Link href={`/bursary/create-bills/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
                  ...
                  {billsData.length > 10 && (
                      <div className="col-12 mb-4 mt-3">
                        <Stack spacing={2} alignItems="end">
                          <Pagination
                            count={Math.ceil(billsData.length / itemsPerPage)}
                            page={page}
