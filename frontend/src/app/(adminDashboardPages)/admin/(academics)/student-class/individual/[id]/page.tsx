...
const IndivivdualStudentClass = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const IndividualDetailsFunction = async () =>{
    try{
      let response = await fetch(`http://school.amanilightequity.com/api/student-class/${id}/`, {
        method: 'GET',
        headers: {
 ...
 console.log(subjectIDs)
  const deleteFunction = async () => {
    ...
    try{
      ...
      if (response.ok) {
        ...
        router.push('/admin/student-class')
        setDeleteModal(false)
        showAlert()
        setIsSuccess(true)
        setMessage('Student Class  deleted')
      } else {
  ...
  const EditDetails = async(e:any) =>{
    ...
    try{
      const response = await fetch(`http://school.amanilightequity.com/api/student-class/${id}/`, {
        method: 'PATCH',
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                              <div className="row g-3">
                                <div className="col-md-6">
                                  <label htmlFor="studentClass" className="form-label">Subject Name</label>
                                  <input type="text" className={`site-input ${errors.studentClass ? 'error-input' : ''}`} {...register('studentClass', {required: true})}  placeholder='Class Name' value={studentClass}  onChange={(e) => setStudentClass(e.target.value)}/>
                  ...
                  <div className="col-12">
                    <div className="site-boxes border-radius-10px">
                      ...
                      <div className='light-text p-3'>
                        <p className='pb-3'>Subjects offer in {formatName(details.name)}</p>
                        <div className="d-flex flex-wrap">
                          {details.subject_name.map((sub:any) =>(
                            <p key={sub.name} className='pe-4 pb-4'>{formatName(sub.name)}</p>
...
export default IndivivdualStudentClass