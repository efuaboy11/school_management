...
const StudentClassPage = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = studentClassData.map((data) => data.id);
  ...
  // const studentClassData = [...Array(100).keys()];
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-student-class/', {
        method: 'POST',
        headers: {
