...
const SessionPage = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = sessionData.map((data) => data.id);
  ...
  // const sessionData = [...Array(100).keys()];
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-session/', {
        method: 'POST',
        headers: {
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/admin/session/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Add session</Link>
          </div>
        </div>
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>
                                <div className="d-flex justify-content-end">
                                  <Link href={`/admin/session/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
