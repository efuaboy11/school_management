...
const StaffNotification = () => {
  ...
  const deleteFunction = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/delete-multiple-staff-notification/', {
        method: 'POST',
        headers: {
  ...
  const UpdateDetails = async (e: any) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/staff-notification/${selectedDataId}/`, {
        method: 'PATCH',
        body: JSON.stringify({
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/admin/staff-notification/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Add notification</Link>
          </div>
        </div>
