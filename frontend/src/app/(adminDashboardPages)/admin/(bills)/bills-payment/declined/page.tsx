...
const DeclinedBills = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = declinedBillsPaymentData.map((data) => data.id);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-bills-payment/', {
        method: 'POST',
        headers: {
      ...
      if(response.ok){
        ...
        setDeclinedBillsPaymentData(declinedBillsPaymentData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === declinedBillsPaymentData.length && declinedBillsPaymentData.length > 0}
