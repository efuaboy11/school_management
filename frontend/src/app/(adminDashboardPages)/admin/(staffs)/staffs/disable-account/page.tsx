...
const DisableStaffAccount = () => {
  ...
  const DisableAccountList = async () =>{
    try{
      let response = await fetch(`http://school.amanilightequity.com/api/disable-account/?user_role=staff`, {
        method: 'GET',
        headers: {
  ...
  const options = staffData.map((student: any) => ({
    value: student.id,
    ...
    <div>
      ...
      <div className="container-lg">
        <div className="d-sm-flex justify-content-between align-center">
          <div>
            <p className="md-text">Disabled Staff Account</p>
            <p className='light-text sm-text mb-4'>Total of {dataCount} staff account disabled</p>
          </div>
...
export default DisableStaffAccount