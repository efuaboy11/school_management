...
const AllTeachers = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = staffData.map((data) => data.id);
  ...
  // const staffData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = staffData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      ...
      if(response.ok){
        ...
        setStaffData(staffData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
    ...
    <div>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">Staffs</p>
            <p className="light-text pb-3">Total of {staffCount} staffs avaliable</p>
         </div>

          <div className='d-flex mb-4'>
            <Link href='/admin/staffs/add' className="site-btn px-3 Link"><i className="ri-user-add-line pe-2"></i> Add  Staff</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="f site-search-input" placeholder="Search" value={staffSearch} onChange={(e) => setStaffSearch(e.target.value)}/>
