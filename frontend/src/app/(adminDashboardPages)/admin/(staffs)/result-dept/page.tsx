...
const ResultDept = () => {
  ...
  const {
    resultOfficerCount,
    resultOfficerData,
    setResultOfficerData,
    resultOfficerLoader,

    resultOfficerSearch,
    setResultOfficerSearch,
    ResultOfficerFunction, 
    FilterResultOfficer,
    ...
    useEffect(() =>{
      if(!resultOfficerSearch){
        ResultOfficerFunction()
      }else if(resultOfficerSearch){
  ...
  console.log(resultOfficerSearch)
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = resultOfficerData.map((data) => data.id);
  ...
  // const resultOfficerData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = resultOfficerData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      ...
      if(response.ok){
        ...
        setResultOfficerData(resultOfficerData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
    ...
    <div>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">Result Dept</p>
            <p className="light-text pb-3">Total of {resultOfficerCount} result officer avaliable</p>
         </div>

          <div className='d-flex mb-4'>
            <Link href='/admin/staffs/add' className="site-btn px-3 Link"><i className="ri-user-add-line pe-2"></i> Add  Result Oficer</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="f site-search-input" placeholder="Search" value={resultOfficerSearch} onChange={(e) => setResultOfficerSearch(e.target.value)}/>
        ...
        <div>
          {resultOfficerLoader ? (
            ...
            <div >
                  ...
                  <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <label className="custom-checkbox cursor-pointer me-3">
                        <input
                          type="checkbox"
                          checked={selectedIDs.length === resultOfficerData.length && resultOfficerData.length > 0}
                ...
                <div className="col-12 ">
                  <div className='site-boxes text-center pb-5 d-flex justify-content-center align-items-center  mt-5 pt-5'>
                    <div>
                      ...
                      <p className='light-text md-text'>No result officer available</p>
                      <p className="light-text">There is no result officer current right now. Check again later</p>
                    </div>
              ...
              {resultOfficerData.length > 10 && (
                <div className="col-12 mb-4">
                  <Stack spacing={2} alignItems="center">
                    <Pagination
                      count={Math.ceil(resultOfficerData.length / itemsPerPage)}
                      page={page}
