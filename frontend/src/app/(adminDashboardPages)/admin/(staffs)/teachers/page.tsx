...
const AllTeachers = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = teacherData.map((data) => data.id);
  ...
  // const teacherData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = teacherData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      ...
      if(response.ok){
        ...
        setTeacherData(teacherData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
    ...
    <div>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">Teachers</p>
            <p className="light-text pb-3">Total of {teacherCount} teachers avaliable</p>
         </div>

          <div className='d-flex mb-4'>
            <Link href='/admin/staffs/add' className="site-btn px-3 Link"><i className="ri-user-add-line pe-2"></i> Add  Teacher</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="f site-search-input" placeholder="Search" value={teacherSearch} onChange={(e) => setTeacherSearch(e.target.value)}/>
                  ...
                  <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <label className="custom-checkbox cursor-pointer me-3">
                        <input
                          type="checkbox"
                          checked={selectedIDs.length === teacherData.length && teacherData.length > 0}
                ...
                <div className="col-12 ">
                  <div className='site-boxes text-center pb-5 d-flex justify-content-center align-items-center  mt-5 pt-5'>
                    <div>
                      ...
                      <p className='light-text md-text'>No Teacher available</p>
                      <p className="light-text">There is no teacher current right now. Check again later</p>
                    </div>
              ...
              {teacherData.length > 10 && (
                <div className="col-12 mb-4">
                  <Stack spacing={2} alignItems="center">
                    <Pagination
                      count={Math.ceil(teacherData.length / itemsPerPage)}
                      page={page}
