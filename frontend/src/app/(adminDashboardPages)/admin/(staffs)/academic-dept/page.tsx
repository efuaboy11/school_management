...
const AcademicDept = () => {
  ...
  const {
    academicOfficerCount,
    academicOfficerData,
    setAcademicOfficerData,
    academicOfficerLoader,

    academicOfficerSearch,
    setAcademicOfficerSearch,
    AcademicOfficerFunction, 
    FilterAcademicOfficer,
    ...
    useEffect(() =>{
      if(!academicOfficerSearch){
        AcademicOfficerFunction()
      }else if(academicOfficerSearch){
  ...
  console.log(academicOfficerSearch)
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = academicOfficerData.map((data) => data.id);
  ...
  // const academicOfficerData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = academicOfficerData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-staff/', {
        method: 'POST',
        headers: {
      ...
      if(response.ok){
        ...
        setAcademicOfficerData(academicOfficerData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
    ...
    <div>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">Academic Dept</p>
            <p className="light-text pb-3">Total of {academicOfficerCount} academic officer avaliable</p>
          </div>

          <div className='d-flex mb-4'>
            <Link href='/admin/staffs/add' className="site-btn px-3 Link"><i className="ri-user-add-line pe-2"></i> Add  Officer</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="f site-search-input" placeholder="Search" value={academicOfficerSearch} onChange={(e) => setAcademicOfficerSearch(e.target.value)}/>
        ...
        <div>
          {academicOfficerLoader ? (
            ...
            <div >
                  ...
                  <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <label className="custom-checkbox cursor-pointer me-3">
                        <input
                          type="checkbox"
                          checked={selectedIDs.length === academicOfficerData.length && academicOfficerData.length > 0}
                    ...
                    <div className="col-lg-4 col-md-6" key={data.id}>
                      <div className="site-boxes border-radius-10px p-3">
                        ...
                        <div className="d-flex justify-content-between light-text">
                          ...
                          <Link href={`/admin/staffs/all/${data.id}`} className='Link light-link'>View details</Link>
                        </div>
                ...
                <div className="col-12 ">
                  <div className='site-boxes text-center pb-5 d-flex justify-content-center align-items-center  mt-5 pt-5'>
                    <div>
                      ...
                      <p className='light-text md-text'>No academic officer available</p>
                      <p className="light-text">There is no academic officer current right now. Check again later</p>
                    </div>
              ...
              {academicOfficerData.length > 10 && (
                <div className="col-12 mb-4">
                  <Stack spacing={2} alignItems="center">
                    <Pagination
                      count={Math.ceil(academicOfficerData.length / itemsPerPage)}
                      page={page}
