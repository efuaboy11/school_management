...
const AllHr = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = hrData.map((data) => data.id);
  ...
  // const hrData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = hrData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () => {
    ...
    try {
      const response = await fetch('http://school.amanilightequity.com/api//delete-multiple-hr/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setHrData(hrData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/admin/hr-dept/add' className="site-btn px-3 Link"><i className="ri-user-add-line pe-2"></i> Add  Hr</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="f site-search-input" placeholder="Search" value={hrSearch} onChange={(e) => setHrSearch(e.target.value)} />
                  ...
                  <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <label className="custom-checkbox cursor-pointer me-3">
                        <input
                          type="checkbox"
                          checked={selectedIDs.length === hrData.length && hrData.length > 0}
                    ...
                    <div className="col-lg-4 col-md-6" key={data.id}>
                      <div className="site-boxes border-radius-10px p-3">
                        ...
                        <div className="wrap-text my-4 site-border border-radius-10px py-2 px-3 site-light-boxes ">
                          ...
                          <div className="flex-wrap d-flex justify-content-between light-text mb-2">
                            <p>Office location:</p>
                            <p>{formatName(data.office_location)}</p>
                          </div>
                        ...
                        <div className="d-flex justify-content-between light-text">
                          ...
                          <Link href={`/admin/hr-dept/all/${data.id}`} className='Link light-link'>View details</Link>
                        </div>
              ...
              {hrData.length > 10 && (
                <div className="col-12 mb-4">
                  <Stack spacing={2} alignItems="center">
                    <Pagination
                      count={Math.ceil(hrData.length / itemsPerPage)}
                      page={page}
