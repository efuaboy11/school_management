...
const IndivivdualPaymentMethod = ({ params }: { params: Promise<{ id: string }> }) => {
  const {id} = use(params)
  ...
  const IndividualDetailsFunction = async () =>{
    try{
      let response = await fetch(`http://school.amanilightequity.com/api/payment-method/${id}/`, {
        method: 'GET',
        headers: {
  ...
  const deleteUserFunction = async () => {
    ...
    try{
      ...
      if (response.ok) {
        ...
        router.push('/admin/payment-method')
        setDeleteModal(false)
        showAlert()
        setIsSuccess(true)
  ...
  const EditDetails = async(e:any) =>{
    ...
    formData.append('name', methodName)
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                              <div className="row g-3">
                                <div className="col-md-6">
                                  <label htmlFor="methodName" className="form-label">Payment method name</label>
                                  <input type="text" className={`site-input ${errors.methodName ? 'error-input' : ''}`} {...register('methodName', {required: true})}  placeholder='Bill Name' value={methodName}  onChange={(e) => setMethodName(e.target.value)}/>
                                  {errors.methodName && <p className="error-text">This field is required</p>}
                                </div>
...
export default IndivivdualPaymentMethod