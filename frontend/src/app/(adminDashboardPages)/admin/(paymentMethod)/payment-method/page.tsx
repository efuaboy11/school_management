...
const PaymentMethod = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = paymentMethodData.map((data) => data.id);
  ...
  // const paymentMethodData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = paymentMethodData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-payment-method/', {
        method: 'POST',
        headers: {
      ...
      if(response.ok){
        ...
        setPaymentMethodData(paymentMethodData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="site-search-input" placeholder="Search" value={paymentMethodSearch} onChange={(e) => setPaymentMethodSearch(e.target.value)}/>
            ...
            <div className='mt-5'>
              {currentItems.length > 0 ? (
                <div>
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === paymentMethodData.length && paymentMethodData.length > 0}
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>{truncateText(data.description, 4)}</td>
                              <td>
                                <Link href={`/admin/payment-method/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
                  ...
                  {paymentMethodData.length > 10 && (
                      <div className="col-12 mb-4 mt-3">
                        <Stack spacing={2} alignItems="end">
                          <Pagination
                            count={Math.ceil(paymentMethodData.length / itemsPerPage)}
                            page={page}
