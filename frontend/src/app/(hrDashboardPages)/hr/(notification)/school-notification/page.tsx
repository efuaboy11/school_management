...
const SchoolNotification = () => {
  ...
  const deleteFunction = async () => {
    ...
    try {
      const response = await fetch('http://school.amanilightequity.com/api/delete-multiple-school-notification/', {
        method: 'POST',
        headers: {
  ...
  const UpdateDetails = async (e: any) => {
    ...
    try {
      const response = await fetch(`http://school.amanilightequity.com/api/school-notification/${selectedDataId}/`, {
        method: 'PATCH',
        body: JSON.stringify({
    ...
    <div className='mb-5 pb-5'>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">School Notifications</p>
            <p className="light-text pb-3">Total of {schoolNotificationCount} notification avaliable</p>
          </div>

          <div className='d-flex mb-4'>
            <Link href='/hr/school-notification/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Add notification</Link>
          </div>
        </div>
