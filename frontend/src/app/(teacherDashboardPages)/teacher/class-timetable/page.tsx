...
const ClassTimetablePage = () => {
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-class-timetable/', {
        method: 'POST',
        headers: {
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/teacher/class-timetable/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Add Timetable</Link>
          </div>
        </div>
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>
                                <Link href={`/teacher/class-timetable/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
