...
const AssignmentSubmissionPage = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = assignmentSubmissionData.map((data) => data.id);
  ...
  const currentItems = assignmentSubmissionData.slice(startIndex, startIndex + itemsPerPage);
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-assignment-submssion/', {
        method: 'POST',
        headers: {
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === assignmentSubmissionData.length && assignmentSubmissionData.length > 0}
                              onChange={handleSelectAll}
                            />
                              <span className="checkmark"></span>
                            </label>
                          </th>                          
                          <th className='py-2'>Student Assignment</th>
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>{formateDateTime(data.date_submitted)}</td>
                              <td>
                                <Link href={`/teacher/assignment-submission/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
