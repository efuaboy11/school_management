...
const AssignmentPage = () => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = assignmentData.map((data) => data.id);
  ...
  // const assignmentData = [...Array(100).keys()];
  const startIndex = (page - 1) * itemsPerPage;
  const currentItems = assignmentData.slice(startIndex, startIndex + itemsPerPage);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      let response = await fetch('http://school.amanilightequity.com/api/delete-multiple-assignment/', {
        method: 'POST',
        headers: {
      ...
      if(response.ok){
        ...
        setAssignmentData(assignmentData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
    ...
    <div className='mb-5 pb-5'>
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          <div>
            <p className="md-text">Assignment</p>
            <p className="light-text pb-3">Total of {assignmentCount} assignment given</p>
         </div>

          <div className='d-flex mb-4'>
            <Link href='/teacher/assignment/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i> Add Assignment</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="site-search-input" placeholder="Search" value={assignmentSearch} onChange={(e) => setAssignmentSearch(e.target.value)}/>
                      ...
                      <thead className='sm-text'>
                        <tr>
                          <th className='py-2'>
                            ...
                            <input
                              type="checkbox"
                              checked={selectedIDs.length === assignmentData.length && assignmentData.length > 0}
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td className='py-3'>{formatName(data.student_class_name.name)}</td>
                              <td>{formatName(data.subject_name.name)}</td>
                              <td>{data.assignment_code}</td>
                              <td>{formatName(truncateText(data.assignment_name, 1))}</td>
                              <td>{data.points}</td>
                              <td>
                                <Link href={`/teacher/assignment/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
                  ...
                  {assignmentData.length > 10 && (
                      <div className="col-12 mb-4 mt-3">
                        <Stack spacing={2} alignItems="end">
                          <Pagination
                            count={Math.ceil(assignmentData.length / itemsPerPage)}
                            page={page}
