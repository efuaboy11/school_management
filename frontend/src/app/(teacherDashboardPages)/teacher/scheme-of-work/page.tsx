...
const SchemeOfWorkPage = () => {
  ...
  useEffect(() =>{
    if(!schemeOfWorkSearch){
      SchemeOFWorkFunction()
    }else if(schemeOfWorkSearch){
    ...
    }, [studentClassQuery, subjectQuery, termQuery])
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const allIDs = schemeOfWorkData.map((data) => data.id);
  ...
  const deleteFunction = async () =>{
    ...
    try{
      ...
      if(response.ok){
        ...
        setSchemeOfWorkData(schemeOfWorkData.filter(dat => dat.id !== selectedIDs))
        setShowDeleteModal(false)
        setSelectedIDs([])
      ...
      <div className="container-xl pt-4">
        <div className="d-md-flex justify-content-between">
          ...
          <div className='d-flex mb-4'>
            <Link href='/teacher/scheme-of-work/add' className="site-btn px-3 Link"><i className="ri-send-plane-fill pe-2"></i>Create scheme</Link>
          </div>
        </div>
        ...
        <div className="d-flex justify-content-end">
          ...
          <div>
            <div className="d-flex align-items-center">
              <input type="text" className="site-search-input" placeholder="Search" value={schemeOfWorkSearch} onChange={(e) => setSchemeOfWorkSearch(e.target.value)}/>
              ...
              <div className="d-sm-flex">
                ...
                <div className="me-3 mb-3">
                  ...
                  <Select
                    ...
                    value={SubjectsOptions.find((opt: { value: string; label: string }) => opt.value === subjectQuery)}
                    onChange={(selectedOption: { value: string; label: string } | null) => setSubjectQuery(selectedOption?.value || '')}
                        ...
                        {currentItems.length > 0 ? (
                          currentItems.map((data) => (
                            <tr key={data.id}>
                              ...
                              <td>
                                <Link href={`/teacher/scheme-of-work/individual/${data.id}`} className="Link site-border box-50px d-flex  align-center justify-content-center border-radius-5px cursor-pointer">
