...
const IndivivdualAssignment = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const IndividualDetailsFunction = async () =>{
    try{
      let response = await fetch(`http://school.amanilightequity.com/api/assignment-submission/${id}/`, {
        method: 'GET',
        headers: {
      ...
      if(response.ok){
        ...
        setTeacherName(data?.teacher_assignment || '')
        setSubjectName(data?.subject || '')
        setAssignmentCode(data?.assignment_code || '')       
        setNote(data?.assignment_note)
        setLoading(false)
      }else{
        setLoading(false)
      }
  ...
  const deleteUserFunction = async () => {
    ...
    try{
      ...
      if (response.ok) {
        ...
        router.push('/student/assignment-submission')
        setDeleteModal(false)
        showAlert()
        setIsSuccess(true)
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                              <div className="row g-3">
                                <div className="col-md-6">
                                  <label htmlFor="lastName" className="form-label">Teacher Assignment<span className="text-danger">*</span> </label>
                                  <Select
                                ...
                                <div className="col-md-12">
                                  <label htmlFor="note" className="form-label">Assignment Note</label>
                                  <textarea rows={6}  className={`site-input ${errors.note ? 'error-input' : ''}`} {...register('note', {required: true})}   value={note}  onChange={(e) => setNote(e.target.value)} placeholder='Instuctions'></textarea>
                                  {errors.note && <p className="error-text">This field is required</p>}
                                </div>
                  ...
                  <div className="col-12">
                    <div className="site-boxes border-radius-10px">
                      <div className="border-bottom1">
                        <div className="p-3">
                          <p>Assignment Submission Note</p>
                        </div>
                        
                      </div>
                      <div className='p-3'>
                        {details.assignment_note ? (
                          <p className='light-text'>{details.assignment_note}</p>
                        ) : (
                          <p className="light-text">No note Placed</p>
                        )}
