...
const IndivivdualSubject = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const IndividualDetailsFunction = async () =>{
    try{
      let response = await fetch(`http://school.amanilightequity.com/api/subjects/${id}/`, {
        method: 'GET',
        headers: {
  ...
  const deleteUserFunction = async () => {
    ...
    try{
      ...
      if (response.ok) {
        ...
        router.push('/academic-officer/subject')
        setDeleteModal(false)
        showAlert()
        setIsSuccess(true)
  ...
  const EditDetails = async(e:any) =>{
    ...
    try{
      const response = await fetch(`http://school.amanilightequity.com/api/subjects/${id}/`, {
        method: 'PATCH',
        body: formData,
        headers:{
          Authorization: `Bearer ${authTokens?.access}`
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                              <div className="row g-3">
                                <div className="col-md-6">
                                  ...
                                  <input type="text" className={`site-input ${errors.subjectName ? 'error-input' : ''}`} {...register('subjectName', {required: true})}  placeholder='Bill Name' value={subjectName}  onChange={(e) => setSubjectName(e.target.value)}/>
                    ...
                    <div className="site-boxes border-radius-10px">
                      ...
                      <div className='light-text p-3'>
                        <div className="pb-3 d-sm-flex justify-content-between">
                          <p className="pb-2 sm-text">Subject name</p>
                          <p>{formatName(details.name)}</p>
                        </div>

                        <div className="pb-3 d-sm-flex justify-content-between">
                          <p className="pb-2 sm-text">section</p>
                          <p>{details.sections}</p>
                        </div>
...
export default IndivivdualSubject